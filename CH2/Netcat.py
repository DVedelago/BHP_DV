...
def execute(cmd): 
    ...
    output = subprocess.check_output(shlex.split(cmd), stderr=subprocess.STDOUT)          # return the output as a string
    return output.decode()



#######################################
# NetCat class and methods definition #
#######################################
...
if __name__ == '__main__': 
   parser = argparse.ArgumentParser(                                                       # (1) Create a CLI to provide arguments
      description='BHP Net Tool',
      formatter_class=argparse.RawDescriptionHelpFormatter,                                # (2) --help invoking
      epilog=textwrap.dedent('''Example:                                                  
                             ...
                             #connect to server
                             '''))
   parser.add_argument('-c', '--command', action='store_true', help='Command_shell')      # (3) Setup interactive shell w/ -c
   parser.add_argument('-e', '--execute', help='Execute specified command')               # -e execute one specific command
   parser.add_argument('-l', '--listen', action='store_true', help='Listen')              # -l listener should be set up
